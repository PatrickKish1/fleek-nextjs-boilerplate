...
import { User, isAddress } from '@/lib/utils';
import { useChat } from '@/lib/hooks/useChat';
import { db } from '@/firebase.config';
import { useAppKitAuth } from '@/lib/hooks/useAppKitAuth';
import { CreateChatFormData } from '@/lib/utils/chatsUtil';
...
export const CreateChat: React.FC<CreateChatProps> = ({
  ...
  const { address, isConnected } = useAppKitAuth();
  ...
  const handleFormSubmit = async (formData: CreateChatFormData) => {
    ...
    try {
      ...
      if (formData.isGroup) {
        ...
        const recipientDoc = await getDoc(doc(db, 'users', formData.recipientAddress));
        ...
        const chatData = {
          ...
          participantAddresses: [address, formData.recipientAddress].map(a => a.toLowerCase()).sort(),
          createdAt: Date.now(),
          updatedAt: Date.now(),
          lastMessage: null,
          unreadCount: {
